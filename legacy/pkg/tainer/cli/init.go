...
func resolveProjectType(t string) (manifest.ProjectType, error) {
	...
	case "react", "cra":
		return manifest.TypeReact, nil
	case "kompozi":
		return manifest.TypeKompozi, nil
	default:
		return "", fmt.Errorf("unknown project type %q\nValid types: wordpress, php, nodejs, nextjs, nuxtjs, nestjs, react, kompozi", t)
	}
}
...
func InitHelp(cmd *cobra.Command, _ []string) {
	...
	lines = append(lines, "  "+flagStyle.Render("--type")+"        "+descStyle.Render("wordpress, php, nodejs, nextjs, nuxtjs, nestjs, react, kompozi"))
	lines = append(lines, "")
