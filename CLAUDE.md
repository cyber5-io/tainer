...
- **Jira project**: TAIN
- **Current version target**: 0.9.0 (in development on `dev/v1`). 1.0.0 reserved for smart pods (aggregate cgroup limits per pod) — depends on cyberstack 0.5.
...
The rebuild is governed by [`docs/superpowers/specs/2026-04-23-tainer-rebuild-cyberstack-design.md`](./docs/superpowers/specs/2026-04-23-tainer-rebuild-cyberstack-design.md). Originally targeted 1.0.0; retargeted to **0.9.0** in Step 8 design to reserve 1.0.0 for smart pods (aggregate cgroup enforcement, requires cyberstack 0.5). CyberStack steps 1-4 have shipped (`v0.1.0` through `v0.4.0` — see status notes in the spec). Steps 5-12 are the tainer rebuild itself, executing now on this branch; Step 13 is the smart-pods capstone (0.5 + tainer 1.0).
