...
> **Status: SHIPPED 2026-05-01 as `cyber-stack v0.3.0`.** All 27 implementation tasks landed; `docker pull / run / exec / logs / inspect / stop / rm / rmi` work end-to-end against `cyberstackd`. Cold-pull P50 = 860ms over n=100 against `mirror.gcr.io/library/alpine`, well inside the 3s budget. Honest gap to OrbStack on cold-pull (P50 467ms on the same machine): +393ms — diagnosed in `cyber-stack/docs/benchmarking.md`, fully sized for 0.4 work. The other four budgets (warm-run, exec, cold-cold, idle) were green at 0.2 and the 0.3 changes are not on their critical paths; full 5-budget re-bench is the first task of 0.4.
>
> **Key deviations from the original plan** (all baked into the v0.3.0 commits):
> 1. **External gvproxy supervisor.** Plan assumed vfkit's in-process gvproxy. Empirically broken under NordVPN's network extension (drops traffic from vfkit's process identity). Shipping launcher spawns standalone gvproxy as a sibling process, mirrors tainer 0.2.4 architecture; `NetworkModeExternalGvproxy` is the default. Three modes available (`external-gvproxy`, `nat`, `socket-vmnet`) gated by `-network-mode`.
> 2. **Storage graph driver: vfs → overlay.** Original plan stayed on vfs for simplicity. Switching to overlay (with `overlay.ko` loaded by init.sh, `e2fsprogs-libs` + `libcom_err` + `libblkid` + `libuuid` + `libeconf` added to the initramfs APK list) saved ~35ms on cold-pull. Required loading the kernel module before agent start.
> 3. **CGO + zig cross-compile for SQLite blob info cache.** Plan assumed `CGO_ENABLED=0` for the static linux-arm64 build. `containers/image` silently degrades to a stub blob-info cache without CGO — material penalty on repeat pulls. Shipping build uses `zig cc -target aarch64-linux-musl` with `-extldflags "-static"`; `mattn/go-sqlite3` enables the real cache.
> 4. **init.sh stderr-blocking root-cause fix.** Late in the cycle, plain `cyberstackd` (no `-console-log`) failed with "network is unreachable" inside the VM. Initially papered over with an "always-on virtio-serial" workaround. Real cause: with virtio-serial absent, `/dev/console` falls back to ttyAMA0 with no host-side reader; init.sh's `set -x` traces and `udhcpc`'s startup writes blocked on the unreachable console, deadlocking DHCP before any DISCOVER could be sent. Shipping fix: `set -x` removed, network-diagnostics block (TAIN-XXXX) deleted, conditional `exec 2>/dev/null` based on `/sys/class/tty/hvc0` presence — verbose only when host explicitly enables `-console-log`. The workaround was reverted.
> 5. **Pull pipeline instrumented + reverted before tag.** httptrace + ReportWriter phase tracer + base64-over-stderr CPU profile harness lived in the agent during the 0.3 endgame to diagnose the OrbStack gap. All three reverted before tagging; the diagnosis (571ms pre-phase, 343ms blob-copy, 98% idle CPU) is preserved in `cyber-stack/docs/benchmarking.md` and seeds the 0.4 design at `tainer/docs/superpowers/specs/2026-05-01-cyberstack-0.4-pull-pipeline-design.md`.
