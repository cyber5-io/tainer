...
3. **CyberStack MVP — container runtime.** `containers/image` + `containers/storage` integration, `crun` subprocess, basic container lifecycle (create/start/stop/rm/exec/logs), networks, volumes, port publishing. ✅ **Tier B done — v0.3.0** (pull/run/exec/logs/inspect/stop/rm/rmi; networks, volumes, port publishing deferred to tier C, ETA post-1.0.0)
